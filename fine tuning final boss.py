...
dataset = load_dataset("AnasVU/career_counselling", split="train")
...
# Copy GGUF files to Google Drive
import shutil
import os
from google.colab import drive
...
# Create GGUF models folder in Google Drive
gguf_dest = '/content/drive/MyDrive/gguf_models'
os.makedirs(gguf_dest, exist_ok=True)

# Copy the GGUF files
print('[INFO] Copying GGUF files to Google Drive...')
for filename in ['phi-2.F16.gguf', 'phi-2.Q4_K_M.gguf']:
    src_path = f'/content/{filename}'
    if os.path.exists(src_path):
        dest_path = os.path.join(gguf_dest, filename)
        print(f'[INFO] Copying {filename}...')
        shutil.copy2(src_path, dest_path)
        file_size = os.path.getsize(dest_path) / (1024**3)  # Convert to GB
        print(f'[INFO] {filename} copied successfully ({file_size:.2f} GB)')
    else:
        print(f'[WARN] {filename} not found at {src_path}')

print('[INFO] All GGUF files copied to:', gguf_dest)
